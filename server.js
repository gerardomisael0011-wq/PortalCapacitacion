...
// Configuración inicial de tablas y reseteo limpio de cursos para que siempre carguen
db.serialize(() => {
    ...
    // Limpiamos y reinsertamos los cursos para asegurar que las nuevas URLs carguen siempre
    db.run("DELETE FROM cursos");
    db.run("INSERT INTO cursos VALUES (1, 'Curso de Seguridad', 'Seguridad', 'video', 'https://www.youtube.com/embed/dQw4w9WgXcQ', 'https://forms.gle/jPLf2fcevrjqAGs1A')");
    db.run("INSERT INTO cursos VALUES (2, 'Manual de Procesos', 'Operaciones', 'pdf', 'https://www.africau.edu/images/default/sample.pdf', 'https://forms.gle/jPLf2fcevrjqAGs1A')");
    db.run("INSERT INTO cursos VALUES (3, 'Presentación ISO', 'Calidad', 'presentacion', 'https://docs.google.com/presentation/d/e/2PACX-1vQ/embed', 'https://forms.gle/jPLf2fcevrjqAGs1A')");

    // Inserción de usuario y asignaciones
...
// Ruta raíz para que nunca falle el GET / y muestre el login directo
app.get('/', (req, res) => {
    res.send(`
    <!DOCTYPE html>
    <html>
...
app.post('/login', (req, res) => {
    ...
    db.get('SELECT * FROM usuarios WHERE nomina = ?', [nomina], (err, user) => {
        // Si no existe el usuario, lo registramos al vuelo para evitar bloqueos
        const procesarLogin = (userData) => {
        ...
        if (!user) {
            const nombreNuevo = `Colaborador Nómina ${nomina}`;
            db.run('INSERT INTO usuarios (nomina, nombre) VALUES (?, ?)', [nomina, nombreNuevo], () => {
                db.run('INSERT OR IGNORE INTO asignaciones (id_usuario, id_curso) VALUES (?, 1), (?, 2), (?, 3)', [nomina, nomina, nomina], () => {
                    procesarLogin({ nomina, nombre: nombreNuevo });
                });
            });
        } else {
            db.run('INSERT OR IGNORE INTO asignaciones (id_usuario, id_curso) VALUES (?, 1), (?, 2), (?, 3)', [nomina, nomina, nomina], () => {
...
app.get('/ver-curso', (req, res) => {
    db.get('SELECT * FROM cursos WHERE id = ?', [req.query.id], (err, c) => {
        ...
        if (c.tipo_contenido === 'video') {
            contenidoHtml = `<iframe src="${c.url_recurso}" width="100%" height="450px" frameborder="0" allowfullscreen style="border-radius: 8px;"></iframe>`;
        } else if (c.tipo_contenido === 'presentacion') {
            contenidoHtml = `<iframe src="${c.url_recurso}" width="100%" height="450px" frameborder="0" style="border-radius: 8px;"></iframe>`;
        } else if (c.tipo_contenido === 'pdf') {
            contenidoHtml = `<iframe src="${c.url_recurso}" width="100%" height="550px" frameborder="0" style="border-radius: 8px;"></iframe>`;
        } else {
            contenidoHtml = `<a href="${c.url_recurso}" target="_blank" style="font-size: 18px; font-weight: bold; color: #0033a0;">Abrir material en una ventana nueva</a>`;
        }
        ...
        <body>
            <div class="card" style="max-width: 900px; margin: auto; padding: 20px;">
                ...
                <div style="text-align: center;">
                    <a href="${c.url_form}" target="_blank" class="btn-primary" style="display: inline-block; padding: 12px 25px; background: #0033a0; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 16px;">ABRIR EXAMEN</a>
                </div>
